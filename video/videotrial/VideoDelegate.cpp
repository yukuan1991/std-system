...
QWidget *VideoDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &, const QModelIndex &index) const
{
...
void VideoDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
    assert (editor);
    auto proxy_model = dynamic_cast<const QSortFilterProxyModel*>(index.model ()); assert (proxy_model);
    auto src_index = proxy_model->mapToSource (index);
...
void VideoDelegate::setModelData(QWidget *editor, QAbstractItemModel *model, const QModelIndex &index) const
{
...
void VideoDelegate::updateEditorGeometry(QWidget *editor, const QStyleOptionViewItem &option, const QModelIndex &) const
{
    editor->setGeometry (option.rect);
}

QWidget *VideoDelegate::create_result_editor(QWidget *parent, const QModelIndex &index, VideoFormModel *src_model) const
{
    assert (index.isValid ());
    assert (src_model);

    auto op_header = src_model->findHorizontalHeader(src_model, index);

    if (op_header == "评比系数")
    {
        auto spinbox = std::make_unique<QDoubleSpinBox> (parent);
...
void VideoDelegate::set_result_editor(QWidget *editor, const QModelIndex &index, VideoFormModel *src_model) const
{
    assert (index.isValid ());
    assert (src_model);
    auto var = index.data ();
...
void VideoDelegate::set_result_model(QWidget *editor, VideoFormModel *src_model, const QModelIndex &index) const
{
    assert (src_model);
    assert (index.isValid ());
